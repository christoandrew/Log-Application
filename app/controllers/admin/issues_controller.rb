class Admin::IssuesController < ApplicationController
  ...
  # GET /issues.json
  def index
    @issues = Issue.includes(:meter_reader).all
  end
  ...
  def create
    ...
    respond_to do |format|
      if @issue.save
        ...
        format.json { render :show, status: :created, location: admin_issue_path(@issue) }
      else
  ...
  def update
    respond_to do |format|
      if @issue.update(issue_params)
        ...
        format.json { render :show, status: :ok, location: admin_issue_path(@issue) }
      else
  ...
  def issue_params
    params.require(:issue).permit(:title, :text, :meter_reader_id)
  end
